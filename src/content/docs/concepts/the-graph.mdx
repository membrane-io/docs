...
Grefs are like URLs for your graph. But unlike URLs, grefs follow a schema and
...
Anything that is referenceable using fields is a node. All the grefs above point
to graph nodes except for the last one that points to an action on the root
node.
