...
Nodes are instances of types in your graph. Anything that is refereceable is a node. To continue with the GitHub
example, a specific `Repository` is a node, but also it's name, the number of stars, as well as the "bio" of the author.
...
| deleteme | <Gref value='github:users.one(name:"membrane-io")' />                                          |
...
Here's an example schema from the <Package name="resend"/> as shown in the IDE.
...
By allowing you to refenrece individual nodes, the graph guarantees that programs (especially ones you didn't write) can
...
- Unifies how data is accessed read regardless of its source (API resources vs. your own abstractions)
