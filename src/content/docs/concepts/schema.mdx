...
Other nodes are referenced by appending a path. For example, if you had <Package name="github" /> running, these would all be valid grefs:
...
For example, the type `Issue` in <Package name="github"/> declares a <Event name="closed"/> event, which is emitted when a
