...
Before learning about schemas, make sure you've read the previous section about [The Graph](/concepts/the-graph) to
understand the core concepts like nodes and grefs.

## Introduction

The types of nodes that a program exposes in its graph are defined in its _schema_.

A program's schema consist of a set of _node types_ with _fields_ that define the available data.

Types can also declare _actions_ which are functions that can be invoked on them, and _events_ which other programs can
be subscribed to.
...
Schemas can be edited from the right sidebar and are stored in `memconfig.json`. You can also edit this file manually
when convenient.
...
node_. The root node serves as the "entry point" into the program, so all grefs are relative to it.
...
Other nodes are referenced by appending a path. For example, if you had [membrane/github](https://membrane.io/share/membrane/github) running, these would all be valid grefs:
...
| <div style={{ maxWidth: "50ex" }}><Gref value="github:users.one(name:'lodash').repos.one(name:'lodash')" /></div> | `Repository`           |
...
As well as a couple of "type wrapper":

- `List<T>` A list of `T` values
- `Gref<T>` A gref to a node of type `T`

These two wrappers are essential for implementing [pagination](/features/pagination) in a unified way across all
programs and drivers.

## Object types

As mention above, programs can define their own types with fields, actions and events.
...
Fields _can_ have a corresponding "resolver" function that gets invoked during queries.
...
For example, the type `PullRequest` in <Package name="github"/> declares an <Action name="merge"/> action which can be invoked on a pull request.
...
For example, the type `Issue` in <Package name="github"/> declares a <Event name="closed"/> which is emitted when a
specific issue is closed.
