...
To illustrate this section, we'll assume that you have <Package name="github" /> running on your workspace, and added the following node as a [connection](/concepts/connections) called `user` to a program:
...
You can click on its type to explore the entire schema.
...
The result of each resolver is passed as the `obj` argument to all children resolvers.
