...
Filtering options for a collection depend on the API itself. Membrane API [packages](/concepts/packages/#public-api-packages) implement pagination in a standard way as described above, but filtering options will vary from package to package.
