...
export async function example4() {
  ...
  const { name, email, repos } = await nodes.user.$query(`
    name
    email
    repos {
      page {
...
But how about querying a collection of repos and paginating through it? For that, we access the `page` field on our repos collection, which itself contains sub-fields for `items<Repository>` and `next<Gref<RepositoryPage>>`.
...
For example, here we're flipping through pages of emilk's repos to find egui (the Rust frontend library that we use at Membrane):

<video src="/videos/pagination.mp4" muted autoplay controls></video>

### Filtering

You may have noticed that the `repos.page` field accepts parameters for filtering. In the case of GitHub repos, you can optionally pass filters for `type`, `sort`, `direction`, `page`, and `pageSize`.

Filtering options for a collection depend on the API itself. Membrane API [drivers](/concepts/drivers) implement pagination in a standard way as described above, but filtering options will vary from driver to driver.

In the explorer sidebar (seen in the video above), you can select filters in the bottom right corner, below the paginated list.
