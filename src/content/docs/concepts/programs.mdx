...
persistence. Each program its its own database.

## Durability

Programs in Membrane are written in TypeScript and run on our cloud runtime. Unlike other serverless
runtimes, Membrane programs are [stateful](/features/state/) which means a few things:
...
Membrane continually and efficienty persists changes to the program's entire memory heap so that when the program goes
...
useful abstractions. We'll learn more about this in the [Graphs](/concepts/the-graph) section.
...
easily build functionality in a variety of ways.
...
"Deploy on save" can be disabled in the settings if needed.
