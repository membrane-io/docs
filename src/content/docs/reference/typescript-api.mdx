...
Programs access most of Membrane's functionality by importing the `"membrane"` module. This module is specific to each program and it's regenerated every time you change its [schema](/concepts/schema) or its [connections](/concepts/connections).
...
You can explore it by cmd+clicking on `"membrane"` to open the generated `membrane.d.ts`. You'll
see the types documented on this page, plus any additional types specific to that program's schema.

:::note
`membrane.d.ts` gets regenerated every time a program's schema or connections change.
:::
...
Use this object to store data. [Durable state](/features/state) is a core concept of Membrane. Learn more about effectively using `state` by reading the [Managing State](/features/state) guide.
...
Every [connection](/concepts/connections) can be accessed via the `nodes` object.

Values in `nodes` have accessors that match the node's [schema](/concepts/schema) so autocomplete works as expected, and
you can access any other node reachable by them.
...
Learn more about [Connections](/concepts/connections) and [Queries](/concepts/queries) in their respective sections.
...
- Implementing `gref` fields for [types that need it](/concepts/queries#gref-resolvers).
- Interacting with the program's own nodes.
...
:::tip
You can also simply `await` an action to invoke it, which is shorthand for calling `.$invoke()`.
:::

#### $invokeAt

Schedules the action to be invoked at a specific time. The invocation time can be either a `Date` object or a UNIX
...
Schedules a cron-like timer that invokes an action at configured times. Unlike traditional cronjob expressions, Membrane
has a resolution of seconds instead of the traditional minutes, so `expression` has an extra slot to specify at which second
...
| month                                                 | 1-12 (or month names: Jan, Feb, Mar, etc)                            |
| day of week                                           | 0-7 (both 0 or 7 represent Sunday, or day names: Sun, Mon, Tue, etc) |
...
[connections](/concepts/connections) available in the `nodes` object, as well as nodes in the program itself (e.g.
`root`, `root.status`, etc).
...
Returns the arguments from the last part of the gref. Only the last element of the gref is considered. Consider
using [`$argsAt`](#argsat) if you need arguments from the middle of the path.
...
Returns the arguments from an arbitrary part of the gref.
The provided `pattern` is also a gref and determines which part to get the arguments from.
...
All events implement this interface. This is not called `Event` to avoid conflicts with other `Event` types in node.js
and browser environments. To unsubscribe from events, use the global [`unsubscribe`](#unsubscribe) function.
...
Returns a promise that resolves after a given number of seconds. You can `await` this promise to temporarily pause the
execution of a function. Thanks to Membrane's durable runtime, you can pause for an arbitrarily long time.
...
Note that, even though you can sleep for an arbitrarily long time, once a function is executing, you can't change it. When building long-running workflows, it's best to use [$invokeIn](#invokein) or [$invokeAt](#invokeat) to schedule workflow steps as separate [actions](/concepts/actions).
